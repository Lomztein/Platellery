...
public class ThrusterModule : Module {
	...
	void Update () {
		GetComponent<AudioSource>().volume = mods[0].value/100f * Game.soundLevel;
		...
		if (fuelContainer) {
			if (isThrusting) {
				if (fuelContainer.fuel > 0) {
					missle.GetComponent<Rigidbody>().AddForceAtPosition (thrustVector.up * thrust * (mods[0].value/100f) * Time.fixedDeltaTime, transform.position);
